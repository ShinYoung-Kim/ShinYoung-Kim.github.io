...
mhnUI = {
    ...
    lock: function() {
        mhnUI.page.hide(), pattern = new PatternLock(".mhn-lock", {
            ...
            $(".mhn-lock-title").html('<span class="mhn-lock-success">잠금이 해제됩니다.</span>'), $(".patt-holder").addClass("patt-success"), setTimeout(function() {
                pattern.reset(), mhnUI.message()
            }, 1e3), mhnUI.page.show()
        }, function() {
            $(".mhn-lock-title").html('<span class="mhn-lock-failure">잘못된 패턴입니다.</span>'), $(".patt-holder").removeClass("patt-success"), setTimeout(function() {
                pattern.reset(), mhnUI.message()
            }, 2e3)
        })
    },
